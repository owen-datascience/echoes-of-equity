...
This script trains a small convolutional neural network with two heads:
- Main head: predicts a binary label (e.g., trustworthy vs neutral).
- Adversarial head: predicts a 3-class group (e.g., ethnicity).

A Gradient Reversal Layer (GRL) is used so that the shared CNN
features become good for the main task but less predictive of the group.
...
def load_data():
    ...
    data_path = base_dir / "data" / "synthetic_adv_cnn_data.npz"
...
def train(num_epochs=15, lambda_grl=1.0):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for xb, yb_main, yb_group in train_loader:
            ...
            # Group prediction with gradient reversal
