...
def compute_acoustic_prosody_features(wav_path: str):
    ...
    try:
        ...
        # Shimmer (local)
        # Note: "Get shimmer (local)" takes 6 arguments: start, end, min_period, max_period, max_period_factor, max_amplitude_factor
        shimmer_local = call(
            [snd, point_process],
            "Get shimmer (local)",
            0.0,            # start time
            ...
            0.03,           # max amplitude factor
        )
        feats["shimmer_local"] = shimmer_local

    except Exception as e:
        print(f"[WARN] Jitter/Shimmer error in {wav_path}: {e}")
